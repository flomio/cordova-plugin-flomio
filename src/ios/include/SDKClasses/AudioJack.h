...
- (void)viewDidLoad
{
    ...
    _acr35 = [[ACRAudioJackReader alloc] init];
    [_acr35 setDelegate:self];
...
// Reset the card.
atr = [_acr35 powerCardWithAction:powerAction slotNum:slotNum timeout:timeout
    error:&error];
if (atr != nil) {

    // Set the protocol.
    activeProtocol = [_acr35 setProtocol:protocols slotNum:slotNum
        timeout:timeout error:&error];

    // Transmit the APDU.
    responseApdu = [_acr35 transmitApdu:commandApdu length:sizeof(commandApdu)
...
controlResponse = [_acr35 transmitControlCommand:controlCommand
...
// Power on the PICC.
[_acr35 piccPowerOnWithTimeout:timeout cardType:cardType];

...

// Transmit the APDU.
[_acr35 piccTransmitWithTimeout:timeout commandApdu:commandApdu
